= Lumen Simple Microservice (Books CURD Api)
...
Each database table has a corresponding *Model* which is used to interact with that table. Models allow you to query for data in your tables, as well as insert new records into the table. For example, one can say `Book::all()` to get all the blog books inside books table rather than writing `select * from books`. Where Book in `Book::all() is a`model`.
...
*6) Books*
Then inside the *app* folder, will create `Book.php`. It is called a model in *MVC framework*.
